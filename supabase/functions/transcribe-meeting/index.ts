...
serve(async (req) => {
  ...
  try {
    ...
    const title = formData.get('title') as string || 'Meeting Recording';
    const participants = JSON.parse(formData.get('participants') as string || '[]');
    const meetingType = formData.get('meeting_type') as string || 'video_conference';
    ...
    if (insertError) {
      ...
      throw new Error(`Failed to create meeting recording record: ${insertError.message}`);
    }

    console.log('Meeting recording record created:', meetingRecording.id);
    ...
    // Update meeting recording with result
    ...
    if (updateError) {
      ...
      throw new Error(`Failed to update meeting recording: ${updateError.message}`);
    }

    console.log('Meeting transcription completed successfully');

    return new Response(
      JSON.stringify({ 
        success: true, 
        meeting_id: meetingRecording.id,
        text: result.text 
      }),
